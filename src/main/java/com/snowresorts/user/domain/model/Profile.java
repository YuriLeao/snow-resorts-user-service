...
public record Profile(
    ...
    public Profile withDetails(String newUsername, String newDisplayName, ShareLevel newShareStats,
                               ShareLevel newShareLocation, Instant now) {
        return new Profile(userId, newUsername, newDisplayName, avatarS3Key, avatarUrl, avatarUpdatedAt,
    ...
    public Profile withAvatar(String newAvatarS3Key, String newAvatarUrl, Instant now) {
        return new Profile(userId, username, displayName, newAvatarS3Key, newAvatarUrl, now,
                lastResortId, shareStats, shareLocation, createdAt, now);
    }

    /** Clear all avatar references. */
    public Profile withoutAvatar(Instant now) {
        return new Profile(userId, username, displayName, null, null, null,
                lastResortId, shareStats, shareLocation, createdAt, now);
    }

    /**
     * A privacy-stripped view of this profile for callers who are not friends:
     * username and display name survive; avatar and stats are hidden.
     */
